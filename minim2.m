function [fun] = minim2(H_exp, H, num0, R0, Rho0, t_exp, P)
    f2 = 0;
    f1 = 0;
    delta = numel(H) - num0;
    test = [];
    for i = 1:num0
        if (i<num0 && i+3+delta<=numel(H) && i+delta+3<numel(P)) && ((H_exp(i) < R0+Rho0) && (P(i-3+delta) == P(i+3+delta))) && (t_exp(i)>=275)
%             if i == 309
%                 P(i-3+delta);
%                 P(i+3+delta);
%             end
    ...
    while i<num0 %&& (H(i) <= R0 || H_exp(i)<=R0)
        if H_exp(i) < R0+Rho0
