...
%     mju = 1;
%     psy = 1;
     [X_min, Err] = fminsearchbnd(@(x)minsearcher3(H_exp, P, s0, Rho0, R0, t, x(1), x(2), x(3), x(4)), [A1, A2, A3, A4], [inf inf inf inf], [inf inf inf inf], optimset('Display', 'iter', 'MaxFunEvals', 3000));
...
%     %syms h_sym
    ...
    for i = 1:numel(P)
        if P(i) ~= Pstart
            %ttt = Testdhdt(k, n, m, P(j), s0, Rho0, R0, Y(end), h_sym);
            %tttt = matlabFunction(ttt);
            [T, Y] = ode45(@(t, h)Testdhdt2(P(j), s0, Rho0, R0, h, X_min(1), X_min(2), X_min(3), X_min(4)), t(j:i), Y(end));
