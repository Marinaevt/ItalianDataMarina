...
Pstart = P1(1);
        ...
        for i = 1:numel(P1)
            if P1(i) ~= Pstart
                [T, Y] = ode45(@(t, h)Testdhdt4_km(P1(j), s0, Rho0, R0, h, sf1, k, m), t1(j:i), Y(end));
                H = [H; Y];
                j = i;
                Pstart = P1(i);
            end
        ...
        for i = 1:numel(P2)
            if P2(i) ~= Pstart
                [T, Y] = ode45(@(t, h)Testdhdt4_km(P2(j), s0, Rho0, R0, h, sf2, k, m), t2(j:i), Y(end));
                H = [H; Y];
                j = i;
                Pstart = P2(i);
            end
        ...
        for i = 1:numel(P3)
            if P3(i) ~= Pstart
                [T, Y] = ode45(@(t, h)Testdhdt4_km(P3(j), s0, Rho0, R0, h, sf3, k, m), t3(j:i), Y(end));
                H = [H; Y];
                j = i;
                Pstart = P3(i);
            end
        ...
        for i = 1:numel(P4)
            if P4(i) ~= Pstart
                [T, Y] = ode45(@(t, h)Testdhdt4_km(P4(j), s0, Rho0, R0, h, sf4, k, m), t4(j:i), Y(end));
                H = [H; Y];
                j = i;
                Pstart = P4(i);
            end
